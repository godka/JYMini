...
int HAPI_DrawStr(lua_State *pL)
{
	...
	int unknown = (int) lua_tonumber(pL, 6);
	const char *fontname=lua_tostring(pL,7);
	int charset=(int)lua_tonumber(pL,8);
	int OScharset = (int) lua_tonumber(pL, 9);
	int flag = (int) lua_tonumber(pL, 10);
	//JY_Debug("%d %d %s %d %d %s %d %d", x, y, str, color, size, fontname, charset, OScharset);
...
int HAPI_LoadPic2(lua_State *pL)
{
    ...
    JY_LoadPicColor2(fileid,picid,picid2,x,y,nooffset,bright);
